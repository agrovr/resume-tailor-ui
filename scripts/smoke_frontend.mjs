...
async function checkPublicShell(baseUrl, signedInCookie = "") {
  ...
  requireCondition(/@media\s*\(min-width:\s*1600px\)\s*\{[\s\S]*?\.templates-page-grid\s*\{(?=[^}]*width:\s*min\(1440px,\s*100%\))(?=[^}]*grid-template-columns:\s*repeat\(3,\s*minmax\(0,\s*1fr\)\))[^}]*\}/s.test(stylesheetText), "templates gallery is missing its wide comparison layout");
  ...
  requireCondition(
    /\.dash-mock\s+\.dash-stats\s*\{(?=[^}]*grid-template-columns:\s*repeat\(auto-fit,\s*minmax\(min\(100%,\s*(?:190|210|230|282)px\),\s*1fr\)\))[^}]*\}/s.test(stylesheetText) ||
      /\.dash-stats\s*\{(?=[^}]*grid-template-columns:\s*repeat\(auto-fit,\s*minmax\(min\(100%,\s*(?:190|210|230|282)px\),\s*1fr\)\))[^}]*\}/s.test(stylesheetText) ||
      /@container\s+dash-main\s*\(min-width:\s*1120px\)\s*\{[^}]*\.dash-mock\s+\.dash-stats\s*\{[^}]*grid-template-columns:\s*repeat\(4,\s*minmax\((?:0|230px),\s*1fr\)\)/s.test(stylesheetText),
    "landing dashboard stats can still squeeze cards at desktop widths",
  );
  requireCondition(/\.dash-mock\s+\.dash-stats\s*\{(?=[^}]*grid-template-columns:\s*repeat\(auto-fit,\s*minmax\(min\(100%,\s*282px\),\s*1fr\)\))[^}]*\}/s.test(stylesheetText), "landing dashboard stats are missing the final roomy card tracks");
  requireCondition(/@container\s+dash-main\s*\(max-width:\s*620px\)\s*\{[^}]*\.dash-mock\s+\.dash-stats\s*\{[^}]*grid-template-columns:\s*1fr/s.test(stylesheetText), "landing dashboard stats are missing the final narrow one-column clamp");
  ...
  requireCondition(/\.dash-stat-value\s*\{(?=[^}]*font-size:\s*clamp\(1rem,\s*4\.1cqi,\s*1\.24rem\))(?=[^}]*line-height:\s*1\.12)(?=[^}]*hyphens:\s*none)(?=[^}]*overflow-wrap:\s*anywhere)(?=[^}]*word-break:\s*normal)[^}]*\}/s.test(stylesheetText), "landing dashboard stat values are missing the final fitted-type override");
  ...
  requireCondition(
    /\.dash-main-head\s+\.btn-sm\s*\{(?=[^}]*min-width:\s*(?:min\(100%,\s*150px\)|172px))(?=[^}]*line-height:\s*1\.12)(?=[^}]*(?:text-wrap:\s*balance|white-space:\s*nowrap))[^}]*\}/s.test(stylesheetText),
  ...
  requireCondition(/\.cta-section\s*\{(?=[^}]*scroll-margin-top:\s*clamp\(128px,\s*9vw,\s*172px\))[^}]*\}/s.test(stylesheetText), "landing final CTA can still anchor beneath the sticky navigation");
  requireCondition(/\.cta-band\s*\{(?=[^}]*padding-block-start:\s*clamp\(72px,\s*6vw,\s*112px\))[^}]*\}/s.test(stylesheetText), "landing final CTA headline can still sit under the sticky navigation");
  ...
  requireCondition(/@media\s*\(max-width:\s*760px\)\s*\{[\s\S]*?\.cta-band,\s*\.cta-band\s+\.cta-cluster\s*\{[^}]*grid-template-columns:\s*1fr/s.test(stylesheetText), "landing final CTA mobile grid can still reserve a hidden visual column");
  requireCondition(/@media\s*\(max-width:\s*760px\)\s*\{[\s\S]*?\.cta-band,\s*\.cta-band\s*>\s*div:first-child,\s*\.cta-band\s+h2,\s*\.cta-band\s+p,\s*\.cta-band\s+\.cta-cluster,\s*\.cta-band\s+\.btn\s*\{(?=[^}]*box-sizing:\s*border-box)(?=[^}]*inline-size:\s*100%)(?=[^}]*max-inline-size:\s*100%)[^}]*\}/s.test(stylesheetText), "landing final CTA mobile text and buttons can still overflow their card");
  requireCondition(
  ...
  requireCondition(finalCtaGuardEndIndex > finalCtaGuardIndex, "landing final CTA screenshot guard was missing its end marker");
  const finalCtaGuard = sourceStylesheetText.slice(finalCtaGuardIndex, finalCtaGuardEndIndex);
  const afterFinalCtaGuard = sourceStylesheetText.slice(finalCtaGuardEndIndex + "/* End final CTA screenshot fix. */".length);
  requireCondition(!/\.cta(?:-|\b)/.test(afterFinalCtaGuard), "landing final CTA screenshot guard is not last in the CTA cascade");
  ...
  requireCondition(/\.cta-visual\s*\{(?=[^}]*inline-size:\s*min\(100%,\s*360px\))(?=[^}]*min-block-size:\s*clamp\(324px,\s*26vw,\s*382px\))[^}]*\}/s.test(finalCtaGuard), "landing final CTA screenshot guard does not contain the final resume artwork box");
  ...
  requireCondition(/@media\s*\(max-width:\s*760px\)\s*\{[\s\S]*?\.settings-page-nav\s*\{(?=[^}]*grid-template-columns:\s*1fr)(?=[^}]*overflow:\s*visible)[^}]*\}[\s\S]*?\.settings-section-list\s*\{(?=[^}]*display:\s*grid)(?=[^}]*grid-template-columns:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\))(?=[^}]*overflow:\s*visible)[^}]*\}[\s\S]*?\.settings-section-list\s+a\s*\{(?=[^}]*line-height:\s*1\.12)(?=[^}]*overflow-wrap:\s*anywhere)(?=[^}]*text-wrap:\s*balance)(?=[^}]*white-space:\s*normal)[^}]*\}/.test(stylesheetText), "settings narrow-tablet nav can still squeeze or clip labels");
...
async function checkBackendCapabilities(backendUrl) {
  ...
  requireCondition(
    Number.isSafeInteger(payload.max_job_description_chars) && payload.max_job_description_chars > 0,
    "backend job-description limit is missing or invalid",
  );
