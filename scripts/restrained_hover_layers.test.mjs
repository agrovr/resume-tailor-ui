...
test("route styles do not duplicate the shared card positioning contract", () => {
  ...
  assert.match(landing, /\.template-card,[\s\S]*?isolation:\s*isolate/);
  assert.match(globals, /\.templates-page-card,[\s\S]*?isolation:\s*isolate/);
});

test("focused interaction feedback remains on the affected marketing surfaces", () => {
  assert.match(combined, /\.step:hover\s*\{[^}]*transform:\s*translateY\(-4px\)/s);
  assert.match(combined, /\.feature-card:hover\s*\{[^}]*transform:\s*translateY\(-4px\)/s);
