...
const publicPagesStyles = readFileSync("app/public-pages.css", "utf8");
...
test("public legal topbar uses a rounded floating command dock", () => {
  ...
  assert.match(globalStyles, /\.legal-topbar::after,\s*\.public-page-topbar::after\s*\{\s*content:\s*none;\s*\}/s);
  assert.match(publicPagesStyles, /\.legal-topbar::after\s*\{\s*content:\s*none;\s*\}/s);
...
test("public legal topbar keeps dark mode and mobile actions shaped safely", () => {
  ...
  assert.match(globalStyles, /html\[data-theme="dark"\]\s+\.legal-topbar::after,\s*html\[data-theme="dark"\]\s+\.public-page-topbar::after\s*\{\s*content:\s*none;\s*\}/s);
  assert.doesNotMatch(publicPagesStyles, /html\[data-theme="dark"\]\s+\.legal-topbar::after\s*\{/);
